...
class HistoryDocUtils:
	...
	def getDumpFilePath(self):
		return os.path.join(os.path.dirname(revitron.DOC.PathName), '{}.dump.json'.format(revitron.DOC.Title))
...
class HistoryEventHandler:
	...
	def debug(self, text):
		if self.debugging:
			...
			file = os.path.join(debugDir, '{}.history.log'.format(self.getCurrentlyActiveDocTitle()))
			...
			if hdu.isTrackedProject():
				...
				if doc.PathName in self.activeDocs:
					...
					for elemId in data['elements']:
						...
						row['transactions'] = json.dumps(data['elements'][elemId]['transactions'])
						# Add current Type to change type event.
						try:
							elem = doc.GetElement(revitron.DB.ElementId(int(elemId)))
							if elem:
								row['transactions'] = row['transactions'].replace('Change Type', 'Change Type to ' + revitron.Parameter(elem, 'Family and Type').getValueString())
						except:
							pass
						rows.append(row)
		...
		if hdu.isTrackedProject() and doc.PathName is not None:
			...
			if transactions not in self.igonredTransactions:
				...
				for id in args.GetModifiedElementIds():
					...
					# Make list unique.
					elements[idStr]['transactions'] = self.uniqueList(elements[idStr]['transactions'])
	...
	def onOpened(self, sender, args):
		...
		if hdu.isTrackedProject():
			...
			if os.path.exists(hdu.getDumpFilePath()):
				...
				if cDateLocal <= cDateDump:
					...
					self.debug('Local file is newer than dump file. Deleting ' + hdu.getDumpFilePath())
					hdu.deleteDumpFile()
		...
		if not self.eventHandlersRegistered:
			if hdu.isTrackedProject():
				...
				self.app.DocumentChanged += EventHandler[DocumentChangedEventArgs](self.onChanged)
				self.app.DocumentSynchronizingWithCentral += EventHandler[DocumentSynchronizingWithCentralEventArgs](self.onSyncing)
				self.app.DocumentSynchronizedWithCentral += EventHandler[DocumentSynchronizedWithCentralEventArgs](self.onSynced)
				self.app.DocumentSaved += EventHandler[DocumentSavedEventArgs](self.onSaved)
